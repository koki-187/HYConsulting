...
async function verifyDataExistence() {
  ...
  console.log('Session 67: 神奈川県20パターンデータ存在検証');
  console.log('='.repeat(80));
  console.log('');
  ...
  for (let i = 0; i < testDataArray.length; i++) {
    ...
    try {
      ...
      const query = sql`
        ...
        FROM transactions
        WHERE prefecture = ${testCase.prefecture}
          AND city LIKE ${`%${testCase.city}%${testCase.district}%`}
          AND type = ${propertyTypeEn}
      `;
      ...
      if (samples.length > 0) {
        ...
        samples.forEach((sample, idx) => {
          console.log(`    ${idx + 1}. ${sample.city} | ${sample.type} | ${sample.trade_price}円 | ${sample.area}㎡ | ${sample.building_year}年築 | ${sample.period}`);
        });
      }
  ...
  console.log(`期待データ総数: ${totalExpected}件`);
  console.log(`実際のデータ総数: ${totalActual}件`);
  console.log(`データ充足率: ${((totalActual / totalExpected) * 100).toFixed(1)}%`);
  console.log('');

  // Save results to JSON
  const outputFile = '/home/ubuntu/hy-consulting-lp/session67-data-verification-results.json';
  fs.writeFileSync(outputFile, JSON.stringify({ 
    summary: { 
      ...
      totalExpected,
      totalActual,
      fulfillmentRate: ((totalActual / totalExpected) * 100).toFixed(1) + '%'
    }, 
    results 
  }, null, 2));
...
function generateMarkdownReport(results, withData, withoutData, totalExpected, totalActual) {
  let markdown = `# Session 67: 神奈川県20パターンデータ存在検証結果\n\n`;
  markdown += `## 検証実施日時\n2026年1月8日\n\n`;
  markdown += `## 検証結果サマリー\n`;
  ...
  markdown += `- **データ充足率**: ${((totalActual / totalExpected) * 100).toFixed(1)}%\n\n`;
  markdown += `---\n\n`;
  ...
  const markdownFile = '/home/ubuntu/hy-consulting-lp/session67-data-verification-results.md';
